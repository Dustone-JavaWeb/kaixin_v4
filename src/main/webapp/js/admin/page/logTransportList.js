$(function() {
	new Vue({
		...
		methods: {
			list: function(start) {
				...
				axios.post(PAGE_LOG_TRANSPORT_QUERY, ajaxRequestModel).then(function(response) {
			...
			showInfo: function(tr) {
				var tabMsg = {
					id: "logTransportInfo" + tr.id,
					eId: "logTransportInfoAera",
					eNum: tr.id,
					name: '转场日志_' + tr.id,
					url: ROUTE_LOG_TRANSPORT_INFO,
					target: 'LogTransportInfo',
					initData: {
						entity: tr
					}
				}
	...
	function vueChooseDate(type, value) {
		logTransportQueryVue.chooseDate(type, value);
	}
